...
def analyze():
    ...
    model_path = 'random_forest_model.pkl'  # Replace with the correct model filename
    ...
    # Prepare user input for Random Forest model
    input_parameters = {
    ...
    # Predict asteroid type using Random Forest model
    manual_prediction = model.predict(manual_input_df)

    # Decode the prediction using the loaded label encoder
    predicted_class_label = label_encoder.inverse_transform([manual_prediction[0]])[0]
